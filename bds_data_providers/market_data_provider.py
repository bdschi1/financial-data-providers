"""Abstract market data provider interface (dict/pandas-based).

Defines the contract that all data providers must implement for the
multi-agent-investment-committee tool layer. Each method returns the
same dict schema regardless of the underlying data source, so tools
can switch providers without code changes.

This ABC is independent of the DataProvider ABC in provider.py,
which serves a different Polars-based interface for backtest-lab
and ls-portfolio-lab.

Concrete implementations:
    - yahoo_market.py    (default, no special requirements)
    - bloomberg_market.py (requires blpapi + Bloomberg Terminal)
    - ib_market.py        (requires ib_insync + TWS/IB Gateway)
"""
...
class MarketDataProvider(ABC):
    ...
    Every provider must implement nine methods that return plain dicts
